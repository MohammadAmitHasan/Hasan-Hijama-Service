        ...
        <nav className='py-2 bg-white z-50 flex container items-center mx-auto '>
            <div className='w-72'>
                ...
                <div className='pt-1 text-center'>
                    <h4 className='text-xl font-semibold'><span className='text-red-400'>Hasan</span> Hijama Services</h4>
                </div>
            </div>
            <div onClick={() => setOpen(!open)} className='w-6 h-6 md:hidden ml-auto mr-4'>
                {open ? <XIcon></XIcon> : <MenuIcon></MenuIcon>}
            </div>
            <ul className={`bg-white text-center md:flex items-center w-full justify-end absolute md:static duration-300 ease-in ${open ? 'top-20' : 'top-[-220px]'}`}>
